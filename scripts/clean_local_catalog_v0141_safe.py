...
def update_food_exact(name, brand, kcal, protein, carbs, fat, sugar, salt, typical_g, purchased, source_note, notes):
    row = con.execute(
    ...
    if row:
        con.execute("""
            ...
            WHERE id=?
        """, (name, brand, kcal, protein, carbs, fat, sugar, salt,
              typical_g, purchased, source_note, notes, row["id"]))
        return row["id"]
...
# Borrar alias exactos que ensucian catálogo.
for bad in [
    "Huevos",
    "Chocolate",
    "cacao",
...
# Si hay duplicados exactos del nombre canonical, dejar el primero.
...
# Notas de hoy, sin tocar otros textos.
con.execute("""
UPDATE meals
